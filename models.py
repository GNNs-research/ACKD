...
class MLP(nn.Module):
    def __init__(
        ...
        if num_layers == 1:
            ...
            self.teacher_feature_encoder = nn.Linear(hidden_dim, output_dim)
            # self.teacher_feature_encoder=GraphConv(hidden_dim, hidden_dim)
            self.mlp_feature_encoder = nn.Linear(hidden_dim, output_dim)
    ...
    def encode_teacher4kd(self, teacher_feat):
        ...
        # return self.dropout(F.relu(self.teacher_feature_encoder(F.relu(self.teacher_feature_encoder(teacher_feat)))))
        # return F.relu(self.teacher_feature_encoder(teacher_feat))

    def encode_mlp4kd(self, mlp_feat):
        return self.dropout(F.relu(self.mlp_feature_encoder(mlp_feat)))
        # return self.dropout(F.relu(self.mlp_feature_encoder(F.relu(self.mlp_feature_encoder(mlp_feat)))))
        # return F.relu(self.mlp_feature_encoder(mlp_feat))
...
class Model(nn.Module):
    ...
    def forward(self, data, feats):
        ...
        if "MLP" in self.model_name:
            # 如果在 self.encoder(data, feats) 后面没有 [1]，那么返回的就是 self.encoder 的完整输出
