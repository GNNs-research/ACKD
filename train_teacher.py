...
from train_and_eval_tea import run_transductive, run_inductive
...
def get_args():
    ...
    parser.add_argument("--num_exp", type=int, default=2, help="Repeat how many experiments")
...
def run(args):
    ...
    if args.exp_setting == "tran":
        ...
        out, score_val, score_test,emb_list,inference_time = run_transductive(
            conf,
            model,
            g,
            feats,
        ...
        out, score_val, score_test_tran, score_test_ind,emb_list,inference_time = run_inductive(
            conf,
            model,
            g,
            feats,
    ...
    teacher_emb_list = emb_list[-1].detach().cpu().numpy()  # last hidden layer
    np.savez(output_dir.joinpath("teacher_emb_list"), teacher_emb_list)
...
def main():
    ...
    if args.num_exp == 1:
        ...
        if args.exp_setting == 'ind':
            # 计算 score_prod，表示将两次实验的得分按照权重 0.8 和 0.2 进行加权平均
    ...
    with open('train_teacher_result.csv', 'a+', newline='') as outFile:
