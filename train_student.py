...
from dataloader import load_data, load_out_t,load_teacher_emb
from utils import (
    get_logger,
    get_evaluator,
    set_seed,
...
from train_and_eval_stu import distill_run_transductive, distill_run_inductive
...
def get_args():
    ...
    parser.add_argument("--adv_distillation_weight", type=float, default=0.04, help="adv_eps for parameter sensitivity")
    parser.add_argument("--adv_eps",type=float,default=0.04,help="adv_eps for parameter sensitivity")
    parser.add_argument("--contrative_distillation", action="store_true",
                        # default=True,
                        help="contrative_distillation", )
    parser.add_argument("--contrative_distillation_weight", type=float, default=0.04, help="contrative_distillation_weight")
    ...
    parser.add_argument("--use_predictor", action="store_true", help="contrative_distillation", )
    ...
    parser.add_argument("--max_epoch", type=int, default=200, help="Evaluate once per how many epochs")
...
def run(args):
    ...
    teacher_emb = load_teacher_emb(out_t_dir)
    logger.info(
    ...
    if args.exp_setting == "tran":
        ...
        out, score_val, score_test,inference_time = distill_run_transductive(
            conf,
            model,
            feats,
            labels,
        ...
        out, score_val, score_test_tran, score_test_ind,inference_time = distill_run_inductive(
            conf,
            model,
            feats,
            labels,
    ...
    # plot_loss_and_score(output_dir.joinpath("loss_and_score.npz"))
    ...
    return score_lst,conf,inference_time
...
def repeat_run(args):
    ...
    for seed in range(args.num_exp):
        args.seed = seed
        score, conf,inference_time = run(args)
        scores.append(score)
        inference_times.append(inference_time)
    scores_np = np.array(scores)
    inference_times_np = np.array(inference_times)
    return scores_np.mean(axis=0), scores_np.std(axis=0),conf,np.mean(inference_times_np)
...
def main():
    ...
    if args.num_exp == 1:
        ...
        score_mean, score_std ,conf,inference_time= repeat_run(args)
        score_str = "".join(
    ...
    with open(args.output_dir.parent.joinpath("exp_results"), "a+") as f:
        f.write(f"tran_score_mean\t ind_score_mean \t tran_score_std \t ind_score_std \t inference_time\n")
        f.write(f"{score_str}\n")
    ...
    if args.exp_setting == 'ind':
        ...
        print(score_prod, end="")
    print(inference_time)
    with open('train_student_result.csv', 'a+', newline='') as outFile:
        writer = csv.writer(outFile, dialect='excel')

        # 如果文件为空，写入表头（参数名）
        if outFile.tell() == 0:
            header = ['Timestamp'] + list(conf.keys())
            if args.num_exp == 1:
                header += ['tran_score_mean']
            elif args.num_exp > 1:
                if args.exp_setting == 'ind':
                    header += [
                        'tran_score_mean', 'tran_score_std',
                        'ind_score_mean', 'ind_score_std', 'score_prod' ,'inference_time'
                    ]
                else:
                    header += [
                        'tran_score_mean', 'tran_score_std','inference_time'
                    ]
        ...
        if args.num_exp == 1:
            ...
            if args.exp_setting == 'ind':
                results += [score_mean[0], score_std[0], score_mean[1], score_std[1], score_prod,inference_time]
            else:
                results += [score_mean[0], score_std[0],inference_time]
